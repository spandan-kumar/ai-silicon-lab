...
void print_help(const char* executable) {
    std::cout
        ...
        << "  --trace-samples N     bounded RVFI samples in result (default 128)\n"
        << "  --cycle-trace-stride N  hash one full-state checkpoint every N cycles (default 1024)\n"
        << "  --execution-trace-stride N  hash every Nth native/RVFI event (default 1: full)\n";
}
...
class MemoryBus {
public:
    explicit MemoryBus(std::uint32_t latency) : memory_(kMemoryBytes, 0), latency_(latency) {}
    ...
    void prepare(Vaisl_soc& top) {
        top.mem_ready = 0;
        top.mem_rdata = 0;
        if (!pending_.active && top.mem_valid) {
            pending_.active = true;
            pending_.address = top.mem_addr;
            pending_.write_data = top.mem_wdata;
            pending_.write_strobe = top.mem_wstrb;
            pending_.instruction = top.mem_instr;
            pending_.remaining = latency_;
        }
        if (!pending_.active) return;
        if (!top.mem_valid || top.mem_addr != pending_.address ||
            top.mem_wdata != pending_.write_data || top.mem_wstrb != pending_.write_strobe ||
            top.mem_instr != pending_.instruction) {
            throw std::runtime_error("external memory request changed before ready");
        }
        const std::uint32_t aligned = pending_.address & ~3u;
        if (aligned > memory_.size() - 4) {
            throw std::runtime_error("external memory request is out of range");
        }
        if (pending_.remaining != 0) {
            --pending_.remaining;
            return;
        }
        top.mem_rdata = read_word(pending_.address);
        top.mem_ready = 1;
    }

    void complete(const Vaisl_soc& top) {
        if (!pending_.active || !top.mem_ready) return;
        const std::uint32_t aligned = pending_.address & ~3u;
        for (unsigned lane = 0; lane < 4; ++lane) {
            if ((pending_.write_strobe >> lane) & 1u) {
                memory_[aligned + lane] = static_cast<std::uint8_t>(pending_.write_data >> (lane * 8));
            }
        }
...
void write_report(const Config& config, const Report& report) {
    ...
    output << "  ],\n"
           ...
           << "    \"soc\": \"aisl_soc\",\n"
           << "    \"cpu\": \"YosysHQ PicoRV32 a473fc8fca393771d83b0ffcf0b14db3393339d8\",\n"
           << "    \"isa\": \"RV32IM\",\n"
           << "    \"external_memory_bytes\": " << kMemoryBytes << ",\n"
           << "    \"memory_latency_cycles\": " << config.memory_latency << ",\n"
...
void hash_cycle(Sha256& hash, const Vaisl_soc& top, std::uint64_t cycle) {
    std::array<std::uint8_t, 39> record{};
    std::size_t offset = 0;
    append_u64(record, offset, cycle);
    append_u8(record, offset, top.mem_valid);
    append_u8(record, offset, top.mem_instr);
    append_u8(record, offset, top.mem_ready);
    append_u32(record, offset, top.mem_addr);
    append_u32(record, offset, top.mem_wdata);
    append_u8(record, offset, top.mem_wstrb);
    append_u32(record, offset, top.mem_rdata);
